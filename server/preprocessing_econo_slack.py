...
def export_questions(qa_df):
    ...
    feature=['client_msg_id','text','user_profile','user','name','ts','replies']
...
def export_answers(qa_df):
    ...
    feature=['client_msg_id','text','user_profile','user','name','parent_user_id','ts']
...
def export_url(text):
    ...
    text = list(map(lambda x: re.findall(pattern=pattern, string=x), text))
    return text
...
def export_question_of_it_share(df):
    df.text = list(map(lambda x: clean_str(x), df.text))
    df['question']=''
    for i in df.index:
        text = df.loc[i,:].text
        if text!='': # URL 지우고 TEXT가 남아있으면 그 내용을 질문에 넣음
            df.question[i]= text
        elif text=='' and not type(df.loc[i,:].attachments==np.float): # URL 지우고 Text가 비었으면 section에서 내용 가져옴
...
def build_url_df(path, path_list, channel='IT관련공유'):
    df = pd.DataFrame()
    for f in path_list:
        f_path = os.path.join(path, f)
        sample_df = pd.read_json(f_path, encoding='utf-8')
        df = pd.concat([df,share_sample_df],axis=0)

    name_df = pd.read_csv(os.path.join(path,'slack-econovation-2018-members.csv'),encoding='utf-8')
    df = name_matching(df,name_df).reset_index(drop=True)
    df['url'] = export_url(df.text)
    df = export_question_of_it_share(df)
    df = construct_dataset_it_share(df,channel)
...
it_share_df = build_qa_df(path_it_share, path_it_share_list, 'IT관련공유')
talk_hard_df = build_qa_df(path_talk_hard, path_talk_hard_list, '기술talk-hard')
'''
